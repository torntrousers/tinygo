...
func (b *builder) createFunctionDefinition() {
	...
	for i, param := range b.fn.Params {
		...
		if b.Debug() && b.fn.Syntax() != nil {
			pos := b.ir.Program.Fset.Position(b.fn.Syntax().Pos())
			diType := b.getDIType(param.Type())
			dbgParam := b.dibuilder.CreateParameterVariable(b.difunc, llvm.DIParameterVariable{
				Name:           param.Name(),
				File:           b.difiles[pos.Filename],
				Line:           pos.Line,
				Type:           diType,
				AlwaysPreserve: true,
				ArgNo:          i + 1,
...
func (b *builder) createExpr(expr ssa.Value) (llvm.Value, error) {
	...
	case *ssa.MakeMap:
		...
		llvmKeyType := b.getLLVMType(mapType.Key().Underlying())
